...
class DataQualityService:
    ...
    def calculate_data_quality_score(
        ...
        price_floor_score = DataQualityService._check_price_floor(
        ...
        spread_score, spread_flag = DataQualityService._check_spread_sanity(
        ...
        volume_score, volume_flag = DataQualityService._check_volume_correlation(
        ...
        volatility_score, volatility_flag = DataQualityService._check_volatility_zscore(
        ...
        stability_score, stability_flag = DataQualityService._check_price_stability(
    ...
    @staticmethod
    def _check_price_floor(buy_price: int, sell_price: int) -> float:
        """
        ...
        if min_price < 10:
            ...
            return 30
        elif min_price < DataQualityService.MIN_ABSOLUTE_PRICE:
            return 60
        else:
            return 100
    ...
    def _check_spread_sanity(
        buy_price: int, sell_price: int, profit: int
        ...
        if spread_multiplier > DataQualityService.MAX_SPREAD_MULTIPLIER:
    ...
    def _check_volume_correlation(
        price_change_pct: float, volume_1h: int
    ) -> Tuple[float, Optional[str]]:
        """
        ...
        if abs_change > 20:
            ...
            expected_min_volume = DataQualityService.MIN_VOLUME_FOR_PRICE_MOVE
    ...
    def _check_volatility_zscore(
        ...
        if historical_volatility is None:
            historical_volatility = DataQualityService._calculate_historical_volatility(item_id)
        ...
        if z_score > DataQualityService.EXTREME_ZSCORE:
            return 20, f"Extreme outlier: {z_score:.1f}σ (>{DataQualityService.EXTREME_ZSCORE}σ typical)"
        elif z_score > DataQualityService.SUSPICIOUS_ZSCORE:
            return 50, f"Suspicious movement: {z_score:.1f}σ (>{DataQualityService.SUSPICIOUS_ZSCORE}σ typical)"
        elif z_score > 1.5:
            return 75, None
        else:
            return 100, None
    ...
    def _calculate_historical_volatility(
        ...
        try:
            ...
            print(f"Error calculating volatility for item {item_id}: {e}")
            return None
    
    @staticmethod
    def _check_price_stability(
        item_id: int, buy_price: int, sell_price: int
    ) -> Tuple[float, Optional[str]]:
        """
        Check if current prices are consistent with recent history.
        ...
        try:
            ...
            print(f"Error checking price stability for item {item_id}: {e}")
            return 70, None
    
    @staticmethod
    def filter_suspicious_items(
        items: List[Dict], min_quality_score: float = 50.0
    ) -> List[Dict]:
        """
