...
export default function HighAlchView() {
  ...
  const SortIndicator = ({ field }) => {
    ...
    return sortOrder === 'asc' ? <span className="text-osrs-gold ml-1">▲</span> : <span className="text-osrs-gold ml-1">▼</span>;
  };

  if (isLoading) {
    return (
      <div className="card text-center py-12 bg-gray-800/50 border border-gray-700 rounded-xl">
        <div className="animate-spin inline-block w-8 h-8 border-4 border-t-osrs-gold border-gray-700 rounded-full mb-4"></div>
        <p className="text-gray-400">Loading profitable alchemy options...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="card text-center py-12 bg-gray-800/50 border border-red-900/50 rounded-xl">
        <span className="text-4xl mb-4 block">⚠️</span>
        <p className="text-red-400 font-bold">Failed to load data: {error.message}</p>
        <button 
          onClick={() => refetch()}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-gray-800 to-gray-850 p-5 rounded-xl border border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-osrs-gold mb-1">🔥 Profitable High Alchemy</h2>
        ...
        <div className="bg-gray-900/60 px-4 py-2.5 rounded-lg border border-gray-800 text-center md:text-right shrink-0">
          <span className="text-xs text-gray-500 block uppercase font-bold tracking-widest">Nature Rune Cost</span>
          <span className="text-lg font-black text-osrs-gold">
          ...
          {/* Note: In alch_service, profit = highalch - buy_price - nat_rune_price => nat_rune_price = highalch - buy_price - profit */}
          <span className="hidden">
            {(() => {
              if (!alchItems || alchItems.length === 0) return 90;
              const first = alchItems[0];
              return first.highalch - first.buy_price - first.profit_per_alch;
            })()}
          </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="relative">
          ...
          <input
            type="text"
            className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-10 pr-4 py-2.5 text-white placeholder-gray-500 focus:border-osrs-gold focus:outline-none transition-colors"
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 flex items-center justify-between">
          <span className="text-sm text-gray-400 font-medium">Min Hourly Volume:</span>
          <div className="flex items-center gap-2">
            <input
              type="number"
              min="0"
              className="w-20 bg-gray-900 border border-gray-700 rounded px-2 py-1 text-center font-bold text-osrs-gold focus:outline-none"
              value={minVolume}
        ...
        <div className="flex gap-2">
          <button
            onClick={() => refetch()}
            className="flex-1 bg-gray-800 hover:bg-gray-700 text-gray-300 font-bold py-2.5 px-4 rounded-lg transition-colors border border-gray-700 flex items-center justify-center gap-2"
          >
            🔄 Refresh Prices
          </button>
        </div>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-900 border-b border-gray-700 text-xs font-bold text-gray-400 uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-gray-750 text-sm">
              {processedItems.map((item) => (
                <tr key={item.id} className="hover:bg-gray-750/30 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      <div>
                        ...
                        <div className="flex gap-1.5 mt-0.5">
                          {item.members && (
                            <span className="text-[10px] bg-blue-900/30 text-blue-400 px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">
                              Members
                            </span>
                          )}
                        </div>
                  ...
                  <td className="py-3.5 px-4 text-right font-medium text-gray-400">
                    ...
                    {item.high_price && item.high_price !== item.buy_price && (
                      <span className="text-xs text-gray-600 block mt-0.5">Instant: {formatNumber(item.high_price)} gp</span>
                    )}
                  </td>
                  <td className={`py-3.5 px-4 text-right font-black ${item.profit_per_alch > 0 ? 'text-osrs-green' : 'text-osrs-red'}`}>
                    ...
                    {item.profit_instant !== null && item.profit_instant !== item.profit_per_alch && (
                      <span className={`text-xs block mt-0.5 font-normal ${item.profit_instant > 0 ? 'text-green-800' : 'text-red-800'}`}>
